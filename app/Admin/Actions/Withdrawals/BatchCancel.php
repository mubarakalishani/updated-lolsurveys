...
class BatchCancel extends Action
{
    protected $selector = '.batch-cancel';
    ...
    public function handle(Request $request)
    {
        ...
        foreach ($withdrawalRequests as $withdrawalRequest) {
            if($withdrawalRequest->status == 2){
                ...
                $withdrawalRequest->update(['status' => 3]);
            }
            
        }
        return $this->response()->success('Selected requests Cancelled Successfully')->refresh();
    }

    public function html()
    {
        return "<a class='batch-cancel btn btn-sm btn-danger show-on-rows-selected d-none me-1 mt-1 mb-1'>Cancel</a>";
    }
}