<?php

namespace App\Admin\Actions\Withdrawals;

use App\Models\User;
use App\Models\WithdrawalHistory;
use Illuminate\Http\Request;
use OpenAdmin\Admin\Actions\Action;

class BatchRefund extends Action
{
    protected $selector = '.batch-refund';

    public function handle(Request $request)
    {
        $keys = explode(',', $request->input('_key'));

        // store each selected ad in ads
        $withdrawalRequests = WithdrawalHistory::whereIn('id', $keys)->get();

        //Pause each task by admin i.e put status to 2
        foreach ($withdrawalRequests as $withdrawalRequest) {
            
            if($withdrawalRequest->status == 2){

            }
            else {
                $withdrawalRequest->update(['status' => 2]);
                $user = User::find($withdrawalRequest->user_id);
                $user->addWorkerBalance($withdrawalRequest->amount_no_fee);
            }
            
        }
        return $this->response()->success('Selected requests Refunded Successfully')->refresh();
    }

    public function html()
    {
        return "<a class='batch-refund btn btn-sm btn-primary show-on-rows-selected d-none me-1 mt-1 mb-1'>Refund</a>";
    }
}